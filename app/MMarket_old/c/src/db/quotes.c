...
static char *path () {
  return path_cat(sys_home(), DATA_PATH, "quotes", NULL);
}
...
Arr *quotes_read(char *nk_name) {
  char *fpath = path_cat(path(), str_f("%s.db", nk_name), NULL);
...
void quotes_modify (char *old_name, char *new_name) {
  char *opath = path_cat(path(), str_f("%s.db", old_name), NULL);
  char *npath = path_cat(path(), str_f("%s.db", new_name), NULL);
...
static Opt *mk_qmtrix (double (*get_value)(Quote *)) {
  ...
  QtableRow *rows = GC_MALLOC(HISTORIC_QUOTES * sizeof(QtableRow));
  QtableRow *p = rows;
  ...
  return opt_new(qtable_new(nicks, rows));
}

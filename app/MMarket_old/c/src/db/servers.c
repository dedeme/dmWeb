...
static char *path () {
  return path_cat(sys_home(), DATA_PATH, "Servers.db", NULL);
}
...
static Servers *read (void) {
  return servers_from_js((Js *)file_read(path()));
}

static void write (Servers *this) {
  file_write(path(), (char *)servers_to_js(this));
}
...
Arr *servers_historic_list (void) {
  int ffilter (Server *sv) {
    ServerConf *conf = opt_oget(server_historic_conf(sv), NULL);
    if (conf) return serverConf_sel(conf) != SERVER_STOPPED;
    return 0;
  }
...
Arr *servers_daily_list (void) {
  int ffilter (Server *sv) {
    ServerConf *conf = opt_oget(server_daily_conf(sv), NULL);
    if (conf) return serverConf_sel(conf) != SERVER_STOPPED;
    return 0;
  }
  return arr_from_it(it_filter(
...
char *servers_acc_url (char *nick) {
  ...
  EACH(servers_list(), Server, sv)
    if (str_eq(server_short_name(sv), ACC_URL)) {
      ...
      if (conf) {
        return str_replace(
          str_replace(serverConf_url(conf), "historico-", ""),
...
void servers_set_conf(int id, int historic, ServerConf *conf) {
  ...
  if (ix != -1) {
    Server *sv = arr_get(list, ix);
    if (historic) {
      if (serverConf_sel(conf) == SERVER_SELECTED) {
        EACH(list, Server, sv)
          Opt *ocf = server_historic_conf(sv);
          if (opt_is_full(ocf)) {
            ServerConf *cf = opt_get(ocf);
            if (serverConf_sel(cf) == SERVER_SELECTED) {
              serverConf_set_sel(cf, SERVER_ACTIVE);
            }
          }
        _EACH
      }

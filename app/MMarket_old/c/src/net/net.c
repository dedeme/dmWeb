...
static Opt *read (ServerConf *conf, char *code) {
  char *url = serverConf_url(conf);
  if (*code) {
    url = str_replace(url, "${code}", code);
  }
  char *html = wwwget(serverConf_cmd(conf), url);
  ...
  int start = goStart(html, 0, str_csplit(serverConf_table_start(conf), '|'));
  if (start == -1) {
    log_error(str_f("Start table not found reading '%s'", url));
    return opt_empty();
  }
  int end = goEnd(html, start, str_csplit(serverConf_table_end(conf), '|'));
  if (end == -1) {
  ...
  Arr *row_start = str_csplit(serverConf_row_start(conf), '|'); // Arr[char]
  Arr *row_end = str_csplit(serverConf_row_end(conf), '|'); // Arr[char]
  Arr *cols_start = arr_new(); // Arr[Arr[char]]
  EACH(serverConf_cols_start(conf), char, mks)
    arr_push(cols_start, str_csplit(mks, '|'));
  _EACH
  Arr *cols_end = arr_new(); // Arr[Arr[char]]
  EACH(serverConf_cols_end(conf), char, mks)
...
Opt *net_server_daily_read(Server *this) {
  ...
  EACH(server_codes(this), ServerCode, sc) {
    ...
    arr_push(r, nickQvalue_new(serverCode_nick_id(sc), dailyEntry_close(de)));
  } _EACH
...
Opt *net_read_daily (ServerConf *conf) {
  ...
  char *types = serverConf_fields_type(conf);
  ...
  EACH(opt_get(table), Arr, row) // row is Arr[char]
    ...
    close = serverConf_is_iso_number(conf)
...
Opt *net_read_historic (ServerConf *conf, char *code) {
  ...
  EACH(opt_get(table), Arr, row) // row is Arr[char]
    ...
    Opt *date = mkdate(
      arr_get(row, ixD),
      serverConf_date_separator(conf),
      serverConf_is_date_eu(conf)
    );
    ...
    if (serverConf_is_iso_number(conf)) {
...
Emsg *net_update_historic(AsyncActor *ac, int nk_id) {
  ...
  EACH(servers, Server, sv)
    ServerConf *conf = opt_get(server_historic_conf(sv));
...
static Map *read_dailyqs (AsyncActor *ac) {
  ...
  if (!ncs || !sv || arr_size(ncs) == 0) {
    ...
    EACH(ncs, NickQvalue, nc)
      Nick *nick = opt_nget(nicks_get(nickQvalue_nick(nc)));
      if (nick) {
        map_put(qs, nick_name(nick), js_wd(nickQvalue_value(nc)));
      } else {
        log_error(str_f(
          "net_update_daily: Nick code '%d' from server '%s' not found",
          nickQvalue_nick(nc), server_name(sv)
        ));
      }
    _EACH
  }
...
static double best_close(Opt *o1, Opt *o2, Opt *o3) {
  double c1 = opt_is_empty(o1) ? -1 : nickQvalue_value(opt_get(o1));
  double c2 = opt_is_empty(o2) ? -1 : nickQvalue_value(opt_get(o2));
  double c3 = opt_is_empty(o3) ? -1 : nickQvalue_value(opt_get(o3));
  ...
  if (
    ...
    void fn () {
      EACH(servers_daily_list(), Server, sv)
        if (
          serverConf_sel(opt_get(server_daily_conf(sv))) ==  SERVER_SELECTED
        ) {
          arr_push(svs, sv);
        }
      _EACH
    ...
    if (arr_size(svs) != 3) {
      ...
      if (arr_size(svncs) == 0) {
        ...
        EACH(arr_get(svncs, 0), NickQvalue, nc)
          int nick_id = nickQvalue_nick(nc);
          ...
          int ffind (NickQvalue *e) { return nickQvalue_nick(e) == nick_id; }

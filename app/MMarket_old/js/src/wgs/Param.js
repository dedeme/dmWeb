...
export default class Param {
  ...
  **/
  constructor (name, min, max, id, nextId) {
    this._name = name;
    this._min = new Dec(min, 4);
    this._max = new Dec(max, 4);
    this._nextId = nextId;
    ...
    this._inp = Ui.field(this._nextId)
      ...
      .value(new Dec((min + max) / 2, 4).toIso())
    ;
  ...
  **/
  get value () {
    const v = this._inp.value().replace(",", ".");

...
class Editor {
  ...
  function serverTests () {
    ...
    It.from(sIdNameCodes).eachSync(
      ...
      () -> {
        setWait("");
        serverTestSpan.removeAll().add(Ui.img(
          result == "" ? "well" : result == "error" ?  "error" : "warning"
        ));
      }
    );
  }
  ...
  function setRightArea (nick: Nick) {
    Cts.client.ssend([
      ...
      final ok = rp["ok"].rb();
      if (!ok) {
        Msg.error(_args(_("Quotes of %0 can not be loaded.<br>See Log."), [nick.name]));
        view();
      }

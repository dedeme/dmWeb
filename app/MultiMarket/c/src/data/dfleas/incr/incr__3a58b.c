...
#define STRIP_TO_SELL 0.015

// Strip is d + d * x where x >= MAX_STRIP && x <= MIN_STRIP
#define MAX_STRIP_BUY 0.30

// Strip is d + d * x where x >= MAX_STRIP && x <= MIN_STRIP
#define MIN_STRIP_BUY 0.10
...
static Arr *fcos(Darr *params, int qnicks, QmatrixValues *closes) {
  return incrBase_cos(qnicks, closes, STRIP_TO_SELL);
}
...
Model *incr__3a58b (void) {
  ...
  arr_push(param_cf, modelMxMn_new(
    str_f("(D. %d, V. %.2f%%)Banda C", DAYS, STRIP_TO_SELL * 100),
    MAX_STRIP_BUY, MIN_STRIP_BUY
  ));

...
static Arr *fcos(Darr *params, int qnicks, QmatrixValues *closes) {
  return incrMMBase_cos(qnicks, closes, darr_get(params, STRIP_TO_SELL));
}

static Order *order(Darr *params, void *company, double q) {
  return incrMMBase_order(
    company,
    q,
...
Model *incrMM__3 (void) {
  ...
  arr_push(param_cf, modelMxMn_new("Banda C", MAX_STRIP_BUY, MIN_STRIP_BUY));
  arr_push(param_cf, modelMxMn_new("Banda V", MAX_STRIP_SELL, MIN_STRIP_SELL));

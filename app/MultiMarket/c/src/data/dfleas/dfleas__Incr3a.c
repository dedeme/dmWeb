...
static Arr *fcos(Darr *params, int qnicks, QmatrixValues *closes) {
  double strip_to_sell = darr_get(params, STRIP_TO_SELL);
  Arr *r = arr_new();
  RANGE0(i, qnicks)
  ...
  if (co->to_sell) {
    if (q > 0) {
      if (new_ref > 0 || co->ref > 0) {
        ...
        if (q <= (co->ref * (1 - darr_get(params, STRIP_TO_SELL)))) {
          co->ref = new_ref;
          co->to_sell = 0;
          return order_sell();
        } else {
    ...
    if (q > 0) {
      if (new_ref > 0 || co->ref > 0) {
        ...
        double ref = co->ref * (1 + darr_get(params, STRIP_TO_BUY));
        if (q >= ref) {
...
Model *dfleas__Incr3a() {
  // Arr[ModelMxMn]
  Arr *param_cf = arr_new();
  arr_push(param_cf, modelMxMn_new("(Días=53) Banda C", MAX_STRIP, MIN_STRIP));
  arr_push(param_cf, modelMxMn_new("(Días=53) Banda V", MAX_STRIP, MIN_STRIP));

...
static Map *test_daily_conf(void *null, Map *rq) {
  CGI_GET_INT(serverId, rq)
  // Map[Js]
  Map *rp = map_new();
  TRY
    map_put(rp, "ok", js_wb(servers_test_daily_conf(serverId)));
  CATCH (ex)
    log_exception(ex);
...
static Map *test_historic_conf(void *null, Map *rq) {
  CGI_GET_INT(serverId, rq)
  CGI_GET_INT(nickId, rq)
  // Map[Js]
  Map *rp = map_new();
  TRY
    map_put(rp, "ok", js_wb(servers_test_historic_conf(serverId, nickId)));
  CATCH (ex)
    log_exception(ex);
    map_put(rp, "ok", js_wb(0));
  _TRY
...
char *sys__servers__download_process(AsyncActor *ac, Map *mrq) {
  ...
  if (str_eq(rq, "activate")) {
    CGI_GET_INT(id, mrq)
    CGI_GET_BOOL(historic, mrq)
    CGI_GET(Rconf *, conf, rconf_from_js, mrq)

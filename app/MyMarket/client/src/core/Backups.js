...
export default class Backups {
    ...
    reader.onloadend = async evt => {
      if (evt.target.readyState === FileReader.DONE) { // DONE == 2
        ...
        if (bindata.length > 0) {
          ...
          } else if (fail === "restore:valid") {
            alert(_("File is not an application backup"));
          } else if (fail === "restore:version") {
            alert(_("File is not a valid version backup"));
          } else {
  ...
  async autorestore (f) {
    ...
    await main.client.send(data);
    alert(_("Auto-backup successfully restored"))
    main.run();
  }
  ...
  async restoreTrash (f) {
    ...
    await main.client.send(data);
    alert(_("Trash backup successfully restored"))
    main.run();
  }

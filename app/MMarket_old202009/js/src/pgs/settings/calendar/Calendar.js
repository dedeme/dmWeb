...
export default class Calendar {
  ...
  **/
  constructor (wg, general, holidays, specialDays) {
    this._wg = wg;
  ...
  general () {
    this._generalDiv.removeAll()
      .add($("table")
        ...
        .add($("tr")
          .add($("td")
            .add(this._gopen.wg.klass("frame")))
          .add($("td")
            .add(this._gclose.wg.klass("frame")))))
    ;
  }
  ...
  **/
  addSpecialDay (ls, date, hopen, mopen, hclose, mclose) {
    ...
    if (It.from(ls).indexf(e => e.date === date) !== -1) {
      Msg.error(_args(_("Date '%0' is duplicated"), date));
      return;
    }
  ...
  **/
  static async mk (wg) {
    ...
    return new Calendar(wg, general, holidays, specialDays);
  }

...
char *noLostPg_process (Mchar *mrq) {
  ...
  if (str_eq(rq, "idata")) {
    ...
    Achar *dates = qs->dates;

    NoLost *noLost = model_noLost(md, qs, params);
    ...
    mchar_put(rp, "dates", achar_to_js(dates));
    mchar_put(rp, "assets", aDouble_to_js(noLost->assets));
    mchar_put(rp, "orders", aOrderNL_to_js(noLost->orders));
    return cgi_rp(rp);

...
func TestCalendar(t *testing.T) {
	...
	minutes := []int{0, 30, 56}
	state := []string{cts.ActSleeping1, cts.ActHistoric, cts.ActSleeping2,
		cts.ActActivating, cts.ActActive, cts.ActDeactivating}
          ...
          prev := calendarTb.PreviousMarketDay(dt).String()
            ...
            if r := eqs(prev, "20211223");  r != "" {
              t.Fatal(r)
					...
					fmt.Println(d, dt, s, calendarTb.IsMarketDay(dt))
				}
			}
		}
	}

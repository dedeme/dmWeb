...
export default class view_Summaries {
  /**
   * @param {!Main} control
   */
  constructor (control) {
  ...
  show () {
    ...
    It.from(Object.keys(pls)).each(k0 => {
      const v0 = pls[k0];
      const pls1 = v0[2];
      let sum0 = 0;
      It.from(Object.keys(pls1)).each(k1 => {
        const v1 = pls1[k1];
        const accs = v1[2];
        let sum1 = 0;
        It.from(Object.keys(accs)).each(ka => {
          sum1 += accs[ka][1];
        });
        v1[1] = sum1;
        sum0 += sum1;
    ...
    It.from(Object.keys(bals)).each(k0 => {
      const v0 = bals[k0];
      const bals1 = v0[2];
      let sum0 = 0;
      It.from(Object.keys(bals1)).each(k1 => {
        const v1 = bals1[k1];
        const accs = v1[2];
        let sum1 = 0;
        It.from(Object.keys(accs)).each(ka => {
    ...
    function statementsSumary () {
      return $("div").style("text-align:center")
        ...
        .add($("table").klass("summary").att("align", "center")
          .adds([...It.from(Object.keys(stats)).sort().map(k => {
            const v = stats[k];
            const desc = v[0];
            const sum = v[1];
            return $("tr")
              .add($("td").klass("summary0cp")
                .add(Ui.link(() => { control.goAcc(k) }).klass("link")
                  .html(k + ". " + desc)))
    ...
    function statementsAccounts () {
      return $("div").style("text-align:center")
        ...
        .add($("table").klass("summary").att("align", "center")
          .adds([...It.from(Object.keys(stats)).sort()
            .reduce(It.empty(), (it, kG) => {
              const vG = stats[kG];
              const descG = vG[0];
              const sumG = vG[1];
              const accs = vG[2];
              return it.push(
                $("tr")
                  .add($("td").att("colspan", 2).klass("summary0cp")
                    .add(Ui.link(() => { control.goAcc(kG) }).klass("link")
                      .html("<b>" + kG + ". " + descG + "</b>")))
                  .add($("td").klass("summary0d")
                    .html("<b>" +
                  ...
                  .add($("td").klass("summary0c")
                    .html("<b>" +
                      (sumG < 0 ? dom.decToStr(new Dec(-sumG, 2)) : "") +
                      "</b>"))
              ).concat(It.from(Object.keys(accs)).sort().map(kA => {
                const vA = accs[kA];
                const descA = vA[0];
                const sumA = vA[1];
                return $("tr")
                  ...
                  .add($("td").klass("summary0cp")
                    .add(Ui.link(() => { control.goAcc(kA) }).klass("link")
                      .html(kA + ". " + descA)))
                  .add($("td").klass("summary0d")
                ...
                $("tr")
                  ...
                  .add($("td").klass("summary0c")
                    .html("<b><i>" +
                      (sumG < 0 ? dom.decToStr(new Dec(-sumG, 2)) : "") +
                      "</i></b>"))
              ).concat(It.from(Object.keys(accs)).sort()
                .reduce(It.empty(), (it, kA) => {
                  const vA = accs[kA];
                  const descA = vA[0];
                  const sumA = vA[1];
                  const subs = vA[2];
                  return it.push($("tr")
                    ...
                    .add($("td").klass("summary0c")
                      .html("<b>" +
                        (sumA < 0 ? dom.decToStr(new Dec(-sumA, 2)) : "") +
                        "</b>"))
                  ).concat(It.from(Object.keys(subs)).sort().map(kS => {
                    const vS = subs[kS];
                    const descS = vS[0];
                    const sumS = vS[1];
                    return $("tr")
                      ...
                      .add($("td").style("width:40px"))
                      .add($("td").klass("summary0cp")
                        .add(Ui.link(() => { control.goAcc(kS) }).klass("link")
    ...
    function plSumary () {
      return $("div").style("text-align:center")
        ...
        .add($("table").klass("summary").att("align", "center")
          .adds([...It.from(Object.keys(pls)).sort()
            .reduce(It.empty(), (it, kGG) => {
              const vGG = pls[kGG];
              const descGG = vGG[0];
              const sumGG = vGG[1];
              const groups = vGG[2];
              return it.push(
                $("tr")
                  .add($("td").att("colspan", 2).klass("summary0cp")
                    .add(Ui.link(() => { control.goAcc(kGG) }).klass("link")
                      .html("<b>" + kGG + ". " + descGG + "</b>")))
                  .add($("td").klass("summary0d")
                    .html("<b>" +
                  ...
                  .add($("td").klass("summary0c")
                    .html("<b>" +
                      (sumGG < 0 ? dom.decToStr(new Dec(-sumGG, 2)) : "") +
                      "</b>"))
              ).concat(It.from(Object.keys(groups)).sort().map(kG => {
                const vG = groups[kG];
                const descG = vG[0];
                const sumG = vG[1];
                return $("tr")
                ...
                $("tr")
                  ...
                  .add($("td").klass("summary0c")
                    .html("<b>" +
                      (sumGG < 0 ? dom.decToStr(new Dec(-sumGG, 2)) : "") +
                      "</b>"))
              ).concat(It.from(Object.keys(groups)).sort()
                .reduce(It.empty(), (it, kG) => {
                  const vG = groups[kG];
                  const descG = vG[0];
                  const sumG = vG[1];
    ...
    function balanceSumary () {
      return $("div").style("text-align:center")
        ...
        .add($("table").klass("summary").att("align", "center")
          .adds([...It.from(Object.keys(bals)).sort()
            .reduce(It.empty(), (it, kGG) => {
              const vGG = bals[kGG];
              const descGG = vGG[0];
              const sumGG = vGG[1];

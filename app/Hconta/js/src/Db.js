...
export default class Db {
  ...
  subgroupsDel (id) {
    this.accountsDel(id);
    this._subgroups = [... It.from(this._subgroups).filter(e => e[0] !== id)];
  }
  ...
  subgroupsMod (modifyId, id, description) {
    ...
    this._subgroups = [... It.from(this._subgroups)
      .map(e => e[0] === modifyId ? [id, description] : e)];
  }
  ...
  subOf (acc) {
    ...
    return self.sub(acc).filter(
      s1 => It.from(self._subaccounts).some(s2 => s2[0].startsWith(s1[0]))
    );
  }
  ...
  diaryDel (ix) {
    this._diary = [... It.from(this._diary)
      .take(ix - 1)
      .concat(It.from(this._diary).drop(ix))
    ];
  }
  ...
  planAnnotations (entryPlan) {
    return It.from(this._diary).reduce(0, (s, e) =>
      It.from(e.debits()).some(d => d.e1.startsWith(entryPlan)) ||
      It.from(e.credits()).some(c => c.e1.startsWith(entryPlan))
        ? s + 1 : s
    );
  }
  ...
  mostUsed () {
    ...
    return [... It.from(Object.keys(accs))
      .sort((a1, a2) => accs[a1] < accs[a2] ? 1 : -1)
      .take(15)];
  }
  ...
  close (year) {
    ...
    return new db_Dentry(
      ...
      "Asiento de apertura",
      [... It.from(Object.keys(accs)).filter(k => accs[k] > 0)
        .map(k => new Tp(k, new Dec(accs[k], 2)))],
      [... It.from(Object.keys(accs)).filter(k => accs[k] < 0)
        .map(k => new Tp(k, new Dec(-accs[k], 2)))]
    );
  }
  ...
  serialize () {
    return JSON.stringify([
      ...
      this._accounts,
      this._subaccounts,
      [... It.from(this._diary).map(e => e.serialize())]
    ]);
  }
  ...
  static restore (serial) {
    ...
    return new Db (
      ...
      pars[1],
      pars[2],
      [... It.from(pars[3]).map(e => db_Dentry.restore(e))]
    );
  }
}

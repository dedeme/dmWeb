...
const dCMarket = "/dm/dmWeb/app/KtWeb/dmcgi/CMarket/acc"

// Each element in entries is: nick, stocks, price
func cMarketData(lastDate string) (entries []string, cash float64) {
	if !file.Exists(dCMarket) {
		panic(dCMarket + " not found")
	}
	...
	for i := 0; i < investors; /*cts.go*/ i++ {
		...
		fdb := path.Cat(
			dCMarket, str.Fmt("Investor-%v", i), "diaries", year)

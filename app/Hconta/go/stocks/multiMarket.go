...
const dMultiMarket = "/home/deme/.dmGoApp/MultiMarket/data/acc"

// Each element in entries is: nick, stocks, price
func multiMarketData(lastDate string) (entries []json.T, cash float64) {
	if !file.Exists(dMultiMarket) {
		panic(dMultiMarket + " not found")
	}
	...
	for i := 0; i < investors; /*cts.go*/ i++ {
		...
		fdb := path.Join(
			dMultiMarket, "Investor-"+strconv.Itoa(i), "diaries", year)

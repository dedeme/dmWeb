...
export  async  function mk(wg)  {sys.$params(arguments.length, 1);
  ...
  arr.sort(Anns, function(A1, A2)  {sys.$params(arguments.length, 2);  return ann.date(A1) < ann.date(A2);});
  ...
  const Show =sys.$checkNull( [[]]);
   ...
   async  function update(id, DayDp, hWg, mWg, txWg)  {sys.$params(arguments.length, 5);
    const Date =sys.$checkNull( datePicker.getDate(DayDp));
    const tx =sys.$checkNull( txWg.getValue().trim());

    if (sys.asBool(!sys.asBool(Date))) {
    ...
    const tm =sys.$checkNull( time.mk(
      time.day(Date[0]), time.month(Date[0]), time.year(Date[0]),
      math.fromStr(hWg.getValue())[0], math.fromStr(mWg.getValue())[0], 0
    ));
    const now =sys.$checkNull( time.now());
    if (sys.asBool(tm - now <= 0))
      Err[0] =sys.$checkExists(Err[0],sys.$checkNull( II("Time is equals or previous to the current one")));
    ...
    await client.ssend({
      ...
      ann: ann.toJs(ann.mk(Id[0], ann.typeFIX, js.w(math.toInt(tm / 1000)), tx))
    });
    mk(wg);
  };
      ...
      .add(Q("td")
        .add(sys.asBool(isNew)
          ? ui.link(function(ev)  {sys.$params(arguments.length, 1); del(Ann.id);})
            .add(ui.img("delete"))
          :sys.asBool( isSel)
            ? ui.link(function(ev)  {sys.$params(arguments.length, 1); update(Ann.id, DayDp, hWg, mWg, txWg);})
              .add(ui.img("enter"))
            : ui.lightImg("delete")))
      .add(Q("td")
        .add(datePicker.mkText(DayDp, dayWg)))

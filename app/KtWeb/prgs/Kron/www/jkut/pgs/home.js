...
export  async  function mk(wg)  {sys.$params(arguments.length, 1);
  ...
  const logDiv =sys.$checkNull( Q("div"));
   ...
   async  function stop(ev)  {sys.$params(arguments.length, 1);
    await client.send({
      prg: cts.appName,
      source: "Home",
      rq: "stop"
      ...
      .add(Q("table")
        ...
        .add(Q("tr")
          .add(Q("td")
            ...
            .text(sys.asBool(isActive) ? II("Active") : II("Stopped"))))
        .add(Q("tr")
          .add(Q("td")
            .style("width:100px;text-align:center")
            .add(ui.link(activate)
              .klass("link")
              .text(sys.asBool(isActive) ? II("Reactivate"): II("Activate"))))
          .add(Q("td")
            .text(" "))
          .add(Q("td")
            .style("width:100px;text-align:center")
            .add(sys.asBool(isActive)
              ? Q("span")
                .add(ui.link(stop)
                  .klass("link")
                  .text(II("Stop")))

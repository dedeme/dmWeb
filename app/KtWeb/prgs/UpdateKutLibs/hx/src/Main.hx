...
import I18n._args;
import data.Lpath;
import pgs.LlistEditor;
import pgs.Editor;

/// UpdateKutLibs depends on the GNU linux programs 'ln' and 'readlink'.
class Main {
  final wg: Domo;
  ...
  public function show (): Void {
    ...
    if (url.exists("0")) {
      try {
        lpath = Some(Lpath.fromJs(Js.from(B64.decode(url.get("0")))));
      } catch (e) {
      }
    }

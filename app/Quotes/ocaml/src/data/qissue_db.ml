...
let set nick_id issue = It.(
  let it = read () |> filter Qissue.(fun i -> i.id <> nick_id) in
  match issue with
  | None -> write it
  | Some iss ->  add0 iss it |> write
)

let get nick_id = read () |> It.find Qissue.(fun i -> i.id = nick_id)

...
// Jail losses historic data base.
package jailLossesDb

import (
	"github.com/dedeme/KtMarket/cts"
	"github.com/dedeme/KtMarket/data/jailLossesEntry"
...
func dpath(investor int) string {
	return path.Cat(rpath(investor), "jailLosses")
}
	...
	for i := 0; i < cts.Investors; i++ {
		if !file.Exists(dpath(i)) {
			...
			file.Write(ypath(i, year), js.Wa([]string{
				jailLossesEntry.New(time.ToStr(d), 0).ToJs(),
			}))
		}
	}
}
...
//      []entries, where each entry is a jailLossesEntry.T ordered from before to
//                 after.
func ReadJs() string {
	ys := Years()
...
//   losses  : Losses (a positive number).
func Add(investor int, losses float64) {
	...
	y := time.Fmt("%Y", d)
	p := ypath(investor, y)
	newAnn := jailLossesEntry.New(ds, rd(losses)).ToJs()
	if file.Exists(p) {
		anns := js.Ra(file.Read(p))
		l1 := len(anns) - 1
		if jailLossesEntry.FromJs(anns[l1]).Date == ds {
			anns[l1] = newAnn
		} else {

...
class Nicks {
  ...
  function new (
    wg: Domo, model: Option<Int>,
    nicks: Array<Nick>, volumes: Map<String, Float>
  ) {
    this.wg = wg;
    this.model = model;
    this.nicks = nicks;
  ...
  function view () {
    ...
    switch (selectedNick) {
      case Some(nick):
        ...
        switch (model) {
          case Some(md):
            for (e in nicks) if (e.id == md) { nickModel = e; break; };
          case None: {}
        }
  ...
  public static function mk (wg: Domo) {
    Cts.client.ssend([
      ...
      final model = rp["model"].isNull() ? None : Some(rp["model"].ri());

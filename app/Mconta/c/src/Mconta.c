...
static char *app_name = "Mconta";
static char *data_version = "201803";
static char *app_dir = "dmcgi/Mconta";
...
*/
static CgiRp *app_process (Cgi *cgi, char *session_id, Map/*Json*/ *rq) {
  char *pg = jmap_gstring(rq, "pg");
  if (!strcmp(pg, "Main")) {
    return pgs_main(cgi, session_id, rq);
  } else if (!strcmp(pg, "Bye")) {
    return pgs_bye(cgi, session_id);
  } else {
    return cgi_error(cgi, str_printf("'%s': Unknown request", pg));
  }
}

/*
static CgiRp *app_processx(Cgi *cgi, char *session_id, Map/ *Json* / *rqm) {
...
int main (int argc, char **argv) {
  ...
  TRY {
    ...
    io_init(cgi, app_name, data_version);

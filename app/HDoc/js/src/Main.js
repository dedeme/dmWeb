...
const app = "HDoc";
const version = "202002"; //eslint-disable-line
...
function bye (client) {
  ...
  const pg = new Msg(app, _args(_("Logout-message"), app));
...
export default class Main {
  ...
  **/
  constructor (client, wgChild = Maybe.nothing, afterShow = Maybe.nothing) {
    ...
    if (wgChild.isNothing()) {
      const auth = new Authentication(app, client);
  ...
  **/
  static async mk () {
    const client = new Client(true, app, () => {
    ...
    if (ok) {
      ...
      const defaultMain = () => {
        const pg = new Paths(app, client, lang, showAll, paths);
        return new Main(
          client,
      ...
      if (url.length === 2) {
        ...
        const pg = await Module.mk(
          client, lib, mod, path.path + "/" + mod + ".hs"
        );
        if (pg.isNothing()) return defaultMain();
      ...
      if (url.length === 3) {
        ...
        if (mod === "") return defaultMain();
        const line = url[2];
        if (isNaN(line)) return defaultMain();
        const pg = await Code.mk(
          client, lib, mod, Number(line), path.path + "/" + mod + ".hs"
        );
        if (pg.isNothing()) return defaultMain();
